...
static int wait_task_zombie(struct wait_opts *wo, struct task_struct *p)
{
    ...
    if (state == EXIT_DEAD){
        ...
        list_for_each_entry_safe(entry, n, &(init->important_list), this_head){
            if (entry->pid == p->pid){
                list_del(&(p->this_head));
                //printk("wait_task_zombie:: deleted pid == %d\n", p->pid);
            }
        }
        //printk("=============== Wait Zombie Ended ===============\n");
        release_task(p);
    }
